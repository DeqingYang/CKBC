...
# 得到语料库，实体嵌入矩阵，关系嵌入矩阵
Corpus_, entity_embeddings, relation_embeddings, Corpus_no_sim, bert_model, train_network, train_data, test_data, all_e1_to_multi_e2 = load_data(args)
...
def evaluate_conv(args, unique_entities):
    ...
    model_conv.eval()
    with torch.no_grad():
        Corpus_no_sim.get_validation_pred(args, model_conv, unique_entities)


def evaluate_conv_v1(args, train_network, test_data, all_e1_to_multi_e2):
    model_conv = SpKBGATConvOnly(bert_model, entity_embeddings, relation_embeddings[:-1, :], args.entity_out_dim,
                                 args.entity_out_dim,
                                 args.drop_GAT, args.drop_conv, args.alpha, args.alpha_conv,
                                 args.nheads_GAT, args.out_channels)
    ...
    model_conv.eval()
    with torch.no_grad():
        Corpus_no_sim.ranking_and_hits(num_rels, model_conv, test_data, all_e1_to_multi_e2, train_network, sim_relations=True)


#train_gat(args)
#train_conv(args)
#evaluate_conv(args, Corpus_no_sim.unique_entities_train)
#evaluate_conv_v1(args, train_network, test_data[0], all_e1_to_multi_e2)