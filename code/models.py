...
class SpKBGATModified(nn.Module):
    ...
    def forward(self, Corpus_, adj, batch_inputs, train_indices_nhop):
        # getting edge list
        edge_list = adj[0] # [train_data[1][0], train_data[1][1]] 尾实体头实体
        edge_type = adj[1] # train_data[1][2] 关系

        edge_list_nhop = torch.cat(
            (train_indices_nhop[:, 3].unsqueeze(-1), train_indices_nhop[:, 0].unsqueeze(-1)), dim=1).t() #实体
        edge_type_nhop = torch.cat(
            [train_indices_nhop[:, 1].unsqueeze(-1), train_indices_nhop[:, 2].unsqueeze(-1)], dim=1) #关系
        ...
        edge_embed = self.relation_embeddings[edge_type] # 该关系嵌入
